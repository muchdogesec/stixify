...
Now use the API and run `POST v1/topics/build_clusters` with `force` = `false` to generate your first clusters. You will need indexed report and the `CLASSIFIER_*` env files must configured in a way that match the number of posts and the types of topics being discussed in them (e.g. if you have a high min cluster size, but wide range of topics, few clusters might end up being created).
