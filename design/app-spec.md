...
https://miro.com/app/board/uXjVKxw0HPw=/
...
Dossiers are groups of STIX Reports. e.g a user could create a Dossier "all reports related to APT1"

```shell
GET HOST/api/VERSION/dosssiers/
```
...
##### POST Create a Dossier

```shell
POST HOST/api/VERSION/dossiers/
```
...
```shell
GET HOST/api/VERSION/dossiers/
```

Accepts URL parameters

* `name` (optional): search name values as wildcard
* `description` (optional): search name values as wildcard
...
```shell
PATCH HOST/api/VERSION/dossiers/
```
